...
#undef LTSIZE
	...
	NICK(s_HelpServ, desc_HelpServ, 0);
    if (s_IrcIIHelp && (!user || stricmp(user, s_IrcIIHelp) == 0))
	NICK(s_IrcIIHelp, desc_IrcIIHelp, 0);
    if (!user || stricmp(user, s_MemoServ) == 0)
	NICK(s_MemoServ, desc_MemoServ, UMODE_o);
    if (!user || stricmp(user, s_OperServ) == 0)
	NICK(s_OperServ, desc_OperServ, UMODE_o | UMODE_i);
#ifdef STATISTICS
    if (!user || stricmp(user, s_StatServ) == 0)
	NICK(s_StatServ, desc_StatServ, UMODE_i);
#endif
    if (s_DevNull && (!user || stricmp(user, s_DevNull) == 0))
	NICK(s_DevNull, desc_DevNull, UMODE_i);
    if (!user || stricmp(user, s_GlobalNoticer) == 0)
	NICK(s_GlobalNoticer, desc_GlobalNoticer, UMODE_o | UMODE_i);
	if (!user || stricmp(user, s_FloodServ) == 0)
	NICK(s_FloodServ, desc_FloodServ, UMODE_i);
}
...
int init(int ac, char **av)
{
	...
	load_ns_dbase();
	if (debug)
	    log("debug: Loaded %s database (1/13)", s_NickServ);
	load_cs_dbase();
	if (debug)
	    log("debug: Loaded %s database (2/13)", s_ChanServ);
    }
    load_os_dbase();
    if (debug)
	log("debug: Loaded %s database (3/13)", s_OperServ);
    load_akill();
    if (debug)
	log("debug: Loaded AKILL database (4/13)");
    load_news();
    if (debug)
	log("debug: Loaded news database (5/13)");
#ifndef STREAMLINED
    load_exceptions();
    if (debug)
        log("debug: Loaded exception database (6/13)");
#else
    if (debug)
        log("debug: Not loading exception database (6/13)");
#endif
#ifdef STATISTICS
    load_ss_dbase();
    if (debug)
	log("debug: Loaded statistics database (7/13)");
#else
    if (debug)
	log("debug: Not loading statistics database (7/13)");
#endif
    log("Databases loaded");

	load_nooper();
	if (debug)
	log("debug: Loaded NOOPER database (8/13)");
    log("Databases loaded");
	
	load_snooper();
	if (debug)
	log("debug: Loaded SNOOPER database (9/13)");
    log("Databases loaded");
	
	load_aconnect();
	if (debug)
	log("debug: Loaded ACONNECT database (10/13)");
    log("Databases loaded");
    
	load_nakill();
	if (debug)
	log("debug: Loaded NAKILL database (11/13)");
    log("Databases loaded");

	load_fs_dbase();
	if (debug)
	log("debug: Loaded FLOODSERV database (12/13)");
    log("Databases loaded");
    
	load_grname_dbase();
	if (debug)
	log("debug: Loaded GRNAME database (13/13)");
    log("Databases loaded");
	...
	/* Rejoin FS channel, if some are in the DB */
	fs_init();
