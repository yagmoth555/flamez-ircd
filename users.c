...
void send_user_list(User *user)
{
	...
	if (u->real_ni)
	    ...
	    strcpy(buf, "-");
	notice(s_OperServ, source, "%s!%s@%s +%s %ld %u %s %s :%s",
	       u->nick, u->username, u->host,
	       mode_flags_to_string(u->mode, MODE_USER), u->signon,
	       u->services_stamp, u->server->name, buf, u->realname);
    }
}
...
void do_umode(const char *source, int ac, char **av)
{
	...
	switch (modechar) {
	  ...
	  case 'a':
		if (is_nooper(av[0], user->username, user->host)==1) break;
		if (is_snooper(av[0], user->server)==1) break;
...
#endif
		if ((is_nooper(av[0], user->username, user->host)==1) || (is_snooper(av[0], user->server)==1)) {
			send_cmd(s_NickServ, "SVSMODE %s :-oOaA", av[0]);
			opcnt--;
#ifdef STATISTICS
			if (user->server)
				user->server->stats->opercnt--;
